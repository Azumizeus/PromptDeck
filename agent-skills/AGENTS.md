...
Ce projet utilise les skills installés dans `.opencode/skills/` (ou un chemin compatible).

## Règles principales

- Si une tâche correspond à un skill, invoquez-le avec l'outil `skill` avant d'agir.
- Les skills se trouvent dans `.opencode/skills/<skill-name>/SKILL.md`.
- Suivez le workflow du skill strictement ; ne l'appliquez pas partiellement.
- Ne sautez jamais les étapes requises comme spec, plan ou test.

## Correspondance intention → skill

- Fonctionnalité / nouvelle fonction → `spec-driven-development`, puis `incremental-implementation` et `test-driven-development`
- Planification / découpage → `planning-and-task-breakdown`
- Bug / échec / comportement inattendu → `debugging-and-error-recovery`
...
1. Déterminez si un skill s'applique (même une petite chance).
2. Chargez le skill avec `skill({ name: "<skill-name>" })`.
3. Suivez le workflow du skill exactement.
4. Ne passez à l'implémentation qu'une fois les étapes requises terminées.
